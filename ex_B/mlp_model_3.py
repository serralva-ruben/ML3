...
class MLP:
    def __init__(self, input_size=2, hidden_size=10, output_size=2, learning_rate=0.01, batch_size=32, seed=42, weight_init_std_dev=0.1):
        self.learning_rate = learning_rate
        self.batch_size = batch_size

        rng = np.random.default_rng(seed)
        
        # Initialize weights with normal distribution based on provided standard deviation
        self.W0 = rng.normal(0, weight_init_std_dev, (hidden_size, input_size))
        self.b0 = np.zeros((hidden_size, 1))
        
        self.W1 = rng.normal(0, weight_init_std_dev, (hidden_size, hidden_size))
        self.b1 = np.zeros((hidden_size, 1))

        self.Wo = rng.normal(0, weight_init_std_dev, (output_size, hidden_size))
        self.bo = np.zeros((output_size, 1))

    def get_activations(self, X):
        # Method to forward propagate through the network and collect activations
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs, plot_interval=5, early_stopping_threshold=0.0001, enable_plotting=True):
        train_losses = []
        val_losses = []

        # Optionally initialize a figure for live plotting if enabled
        if enable_plotting:
            plt.figure(figsize=(8, 6))
            live_plot_figure = plt.gcf()

        # Collect activations after initialization and before training
        initial_activations = self.get_activations(X_val)

        halfway_activations = None  # Placeholder for activations in the middle of training
        final_activations = None  # Placeholder for activations at the end of training
        halfway_epoch = epochs // 2  # Define the halfway epoch
        ...
        for epoch in range(epochs):
            ...
            if epoch > 0 and abs(val_losses[-1] - val_losses[-2]) < early_stopping_threshold:
                ...
                if halfway_activations is None:  # If we haven't reached halfway, capture now
                    halfway_activations = self.get_activations(X_val)
                final_activations = self.get_activations(X_val)
                break

            # Live plotting if enabled
            if enable_plotting and epoch % plot_interval == 0:
                live_plot_figure.clf()
                self.plot_decision_boundaries(X_val, y_val, epoch)
                live_plot_figure.canvas.draw()
                plt.pause(0.01)

            # Collect activations in the middle of training
        ...
        # Ensure final activations are set if not already done so by early stopping
        if final_activations is None:
            final_activations = self.get_activations(X_val)

        # Return the training and validation losses, and the activations
        return train_losses, val_losses, (initial_activations, halfway_activations, final_activations)
